...
// This method is used to forward a Transition Barrier structure to the ResourceStateTracker::ResourceMethod
// The transition barrier method doesn't need the before state as the ResourceStateTracker will handle it when
// committing the resource barriers.
...
// Forward a UAV Barrier structure to the Resource State Tracker
...
// Forward a Aliasing Barrier structure to the Resource State Tracker
                                   ...
                                   Microsoft::WRL::ComPtr<ID3D12Resource> afterResource, bool flushBarriers )
{
    ...
    m_ResourceStateTracker->ResourceBarrier( barrier );
    // If the resource barriers should be committed to the command list. Flush the resource barriers.
    if ( flushBarriers )
    {
        FlushResourceBarriers();
    }
...
// Flush non-pending resource barriers to the command list
                                ...
                                Microsoft::WRL::ComPtr<ID3D12Resource> srcRes )
{
    ...
    // Before copying the contents of one resource to another the destination and source need to be set to the right
    // copy state
    TransitionBarrier( dstRes, D3D12_RESOURCE_STATE_COPY_DEST );
    TransitionBarrier( srcRes, D3D12_RESOURCE_STATE_COPY_SOURCE );

    // Commit resource barriers to the command list.
    FlushResourceBarriers();

    // Copy the source resource to the destination resource
    m_d3d12CommandList->CopyResource( dstRes.Get(), srcRes.Get() );

    // Ensure the lifetime of both resources by adding them to a list of tracked resources.
    // Ensure that neither of the resources go out of scope.
    TrackResource( dstRes );
    TrackResource( srcRes );
}
// The CopyResource method is used to copy one GPU resource to another
...
// Ensure all the subresource are in the correct state before letting them be used by the command list.
                                      ...
                                      uint32_t dstSubresource, uint32_t srcSubresource )
{
    ...
    // Transition all subresources to the same state
    ...
    // Resolve the resource into non multi-sampled resource
...
// This method uses the DynamicDescriptorHeap class to stage an SRV to a GPU visible descriptor heap. It also
// transitions the resource to the correct state for use as an SRV on the graphics or compute pipelines.
                                         ...
                                         D3D12_RESOURCE_STATES stateAfter, size_t bufferOffset )
{
    if ( buffer )
    {
        ...
        // The SRV matching the SRV description is retrieved from the resource and stage to the DynamicDescriptorHeap
        // using StageDescriptors.
        ...
        // Ensure the lifetime of the resource by adding it to a list of tracked resources.
        TrackResource( buffer );
    }
}
/*
 * The methods below function in the same principle as the ShaderResourceView.
 * Each method provides a way to create an object as a resource for each stage in the graphics pipeline and
 * provides tracking reference for that object.
 */
...
// The Draw method is used to render geometry to the currently bound render target. Before executing a Draw command on
// the command list, all barriers must be flushed to the command list and any resource descriptors that were staged to
// the DDH need to be committed.
void CommandList::Draw( uint32_t vertexCount, uint32_t instanceCount, uint32_t startVertex, uint32_t startInstance )
{
    FlushResourceBarriers();
    for ( int i = 0; i < D3D12_DESCRIPTOR_HEAP_TYPE_DSV; ++i)  // This was the wrong flag I used to index through
    for ( int i = 0; i < D3D12_DESCRIPTOR_HEAP_TYPE_NUM_TYPES; ++i )// The fix 
    {//Ensure all of the resource descriptors are bound to the graphics pipeline
