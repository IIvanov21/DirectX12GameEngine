...
// The constructor initializes the variables for the DDH and allocates storage for the descriptor handle cache based
// on the maximum number of descriptors per descriptor heap
...
, m_NumFreeHandles( 0 )
{
    // Query the increment size of the descriptor
...
// This method is used to configure the layout of the descriptor cache whenever the root
...
// This method prepares the CPU descriptor handles for committing them to the GPU visible descriptors
...
{
   //The preconditions of the arguments are checked to ensure the user is not able to copy more descriptors
    //than it can fit in the descriptor heap or tries to set the descriptors to an invalid index in the descriptor table cache.
