...
DirectX12HDR::DirectX12HDR( const std::wstring& name, int width, int height, bool vSync )
...
, m_RenderScale( 1.0f )
{
    ...
    m_Window->Update += UpdateEvent::slot( &DirectX12HDR::OnUpdate, this );
    m_Window->KeyPressed += KeyboardEvent::slot( &DirectX12HDR::OnKeyPressed, this );
    m_Window->KeyReleased += KeyboardEvent::slot( &DirectX12HDR::OnKeyReleased, this );
    m_Window->MouseMoved += MouseMotionEvent::slot( &DirectX12HDR::OnMouseMoved, this );
    m_Window->MouseWheel += MouseWheelEvent::slot( &DirectX12HDR::OnMouseWheel, this );
    m_Window->Resize += ResizeEvent::slot( &DirectX12HDR::OnResize, this );
    m_Window->DPIScaleChanged += DPIScaleEvent::slot( &DirectX12HDR::OnDPIScaleChanged, this );
...
DirectX12HDR::~DirectX12HDR()
{
    _aligned_free( m_pAlignedCameraData );
}

uint32_t DirectX12HDR::Run()
{
    LoadContent();
...
bool DirectX12HDR::LoadContent()
{
    ...
    m_BeKind                = commandList->LoadTextureFromFile( L"Assets/Textures/be_great.jpg", true );
    m_MinecraftTexture      = commandList->LoadTextureFromFile( L"Assets/Textures/Minecraft.jpg", true );
    m_MarsTexture          = commandList->LoadTextureFromFile( L"Assets/Textures/mars.jpg", true );
    m_MineLisaTexture       = commandList->LoadTextureFromFile( L"Assets/Textures/Mine_Lisa.jpg", true );
    m_EveningSun            = commandList->LoadTextureFromFile( L"Assets/Textures/EveningSun.hdr", true );


    // Create a cubemap for the HDR panorama.
    auto cubemapDesc  = m_EveningSun->GetD3D12ResourceDesc();
    cubemapDesc.Width = cubemapDesc.Height = 1024;
    cubemapDesc.DepthOrArraySize           = 6;
    cubemapDesc.MipLevels                  = 0;

    m_EviningSunCubemap = m_Device->CreateTexture( cubemapDesc );
    m_EviningSunCubemap->SetName( L"Grace Cathedral Cubemap" );

    // Convert the 2D panorama to a 3D cubemap.
    commandList->PanoToCubemap( m_EviningSunCubemap, m_EveningSun );
    ...
    m_EviningSunCubemapSRV = m_Device->CreateShaderResourceView( m_EviningSunCubemap, &cubeMapSRVDesc );
    ...
    // Create a root signature and PSO for the skybox shaders.
    {
        ...
        ComPtr<ID3DBlob> vs;
        ComPtr<ID3DBlob> ps;
        ThrowIfFailed( D3DReadFileToBlob( L"data/shaders/DirectX12EngineHDRSample/Skybox_VS.cso", &vs ) );
        ThrowIfFailed( D3DReadFileToBlob( L"data/shaders/DirectX12EngineHDRSample/Skybox_PS.cso", &ps ) );
    ...
    // Create a root signature for the HDR pipeline.
    {
        ...
        ComPtr<ID3DBlob> vs;
        ComPtr<ID3DBlob> ps;
        ThrowIfFailed( D3DReadFileToBlob( L"data/shaders/DirectX12EngineHDRSample/HDR_VS.cso", &vs ) );
        ThrowIfFailed( D3DReadFileToBlob( L"data/shaders/DirectX12EngineHDRSample/HDR_PS.cso", &ps ) );
        ...
        ThrowIfFailed( D3DReadFileToBlob( L"data/shaders/DirectX12EngineHDRSample/Unlit_PS.cso", &unlitPixelShader ) );
    ...
    // Create the SDR Root Signature
    {
        ...
        ComPtr<ID3DBlob> vs;
        ComPtr<ID3DBlob> ps;
        ThrowIfFailed( D3DReadFileToBlob( L"data/shaders/DirectX12EngineHDRSample/HDRtoSDR_VS.cso", &vs ) );
        ThrowIfFailed( D3DReadFileToBlob( L"data/shaders/DirectX12EngineHDRSample/HDRtoSDR_PS.cso", &ps ) );
...
void DirectX12HDR::RescaleHDRRenderTarget( float scale )
{
...
void DirectX12HDR::OnResize( ResizeEventArgs& e )
{
...
void DirectX12HDR::OnDPIScaleChanged( DPIScaleEventArgs& e )
{
    m_GUI->SetScaling( e.DPIScale );
}

void DirectX12HDR::UnloadContent()
{
    m_Sphere.reset();
    m_Cone.reset();
    m_Torus.reset();
    ...
    m_MinecraftTexture.reset();
    m_MarsTexture.reset();
    m_MineLisaTexture.reset();
    m_EveningSun.reset();
    m_EviningSunCubemap.reset();
    m_BeKind.reset();
...
void DirectX12HDR::OnUpdate( UpdateEventArgs& e )
{
...
void DirectX12HDR::OnGUI( const std::shared_ptr<DX12_Library::CommandList>& commandList,
                       const DX12_Library::RenderTarget&                 renderTarget )
{
...
void DirectX12HDR::OnRender()
{
    ...
    // Render the skybox.
    {
        ...
        commandList->SetShaderResourceView( 1, 0, m_EviningSunCubemapSRV,
    ...
    commandList->SetShaderResourceView( RootParameters::Textures, 0, m_MarsTexture,
                                        D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE );
    ...
    commandList->SetGraphicsDynamicConstantBuffer( RootParameters::MatricesCB, matrices );
    commandList->SetGraphicsDynamicConstantBuffer( RootParameters::MaterialCB, Material::White );
    commandList->SetShaderResourceView( RootParameters::Textures, 0, m_MineLisaTexture,
                                        D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE );
    ...
    commandList->SetGraphicsDynamicConstantBuffer( RootParameters::MatricesCB, matrices );
    commandList->SetGraphicsDynamicConstantBuffer( RootParameters::MaterialCB, Material::White );
    commandList->SetShaderResourceView( RootParameters::Textures, 0, m_MinecraftTexture,
                                        D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE );
    ...
    commandList->SetGraphicsDynamicConstantBuffer( RootParameters::MatricesCB, matrices );
    commandList->SetGraphicsDynamicConstantBuffer( RootParameters::MaterialCB, Material::White );
    commandList->SetShaderResourceView( RootParameters::Textures, 0, m_BeKind,
                                        D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE );
...
void DirectX12HDR::OnKeyPressed( KeyEventArgs& e )
{
...
void DirectX12HDR::OnKeyReleased( KeyEventArgs& e )
{
    if ( !ImGui::GetIO().WantCaptureKeyboard )
    {
        switch ( e.Key )
...
void DirectX12HDR::OnMouseMoved( MouseMotionEventArgs& e )
{
...
void DirectX12HDR::OnMouseWheel( MouseWheelEventArgs& e )
{
    if ( !ImGui::GetIO().WantCaptureMouse )
    {
